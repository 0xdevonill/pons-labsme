...
import { emptySocials, type GraduationPhase, type TokenMeta, type V1Launch, type V2Launch } from "@/lib/types";
...
export function useTokenDetail(address?: Address) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!client || !address) throw new Error("missing");
      ...
      if (v2.exists) {
        const [info, name, symbol, curveState, policy] = await Promise.all([
        ...
        const [realQuote, sellable, feeBps, creatorTaxBps, ready, graduated, snipe] =
          await Promise.all([
        ...
        const meta: TokenMeta = {
          ...
          socials: {
            ...
            telegram: info[3].telegram,
            discord: info[3].discord,
            website: info[3].website,
            farcaster: info[3].farcaster,
          },
        };
