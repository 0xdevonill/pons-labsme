      ...
      {query.isError ? (
        ...
        <div className="space-y-2">
          {rows.slice(0, 80).map((row) => (
            <div key={row.txHash} className="glass flex items-center justify-between rounded-2xl px-4 py-3 text-sm">
              <Link href={`/token/${row.token}`} className="font-medium">
                {row.generation.toUpperCase()} {shorten(row.token)}
              </Link>
              <span className="text-[var(--muted)]">{pairSymbol(row.pairToken)}</span>
              <a href={explorerTx(row.txHash)} target="_blank" rel="noreferrer" className="text-[var(--muted)]">
