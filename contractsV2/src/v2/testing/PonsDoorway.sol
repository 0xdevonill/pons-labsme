 ...
 *
 * @dev
 * This contract is an architectural reference implementation for the Pons Doorway.
 * It models the lifecycle of liquidity migration requests between:
 *
 *      Solana <-> Pons Doorway <-> Robinhood Chain
 *
 ...
 *
 * Pons Doorway is designed around the same modular philosophy used throughout
...
contract PonsDoorway {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "PonsDoorway: not owner");
        _;
    }

    modifier onlyRelayer() {
        require(msg.sender == relayer, "PonsDoorway: not relayer");
        _;
    }

    modifier onlyGuardian() {
        require(msg.sender == guardian, "PonsDoorway: not guardian");
        _;
    }

    modifier doorwayOpen() {
        require(doorwayActive, "PonsDoorway: paused");
        _;
    }
        ...
        returns (bytes32 migrationId)
    {
        require(
            ...
            "PonsDoorway: token unsupported"
        );

        require(
            supportedSolanaMints[solanaMint],
            "PonsDoorway: mint unsupported"
        );

        require(amount > 0, "PonsDoorway: zero amount");
        ...
        external
    {
        ...
        require(
            ...
            msg.sender == guardian,
            "PonsDoorway: not authorized"
        );

        require(
            migration.status == MigrationStatus.REQUESTED ||
            migration.status == MigrationStatus.ATTESTED,
            "PonsDoorway: cannot cancel"
        );

        require(
            block.timestamp >=
                migration.createdAt + CANCELLATION_DELAY,
            "PonsDoorway: cancellation locked"
        );
