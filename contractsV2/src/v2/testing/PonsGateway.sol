...
/**
 * @title PonsGateway
 * @notice Cross-chain liquidity migration gateway between Robinhood Chain and Solana.
 *
 * @dev
 * This contract is an architectural reference implementation for the Pons Gateway.
 * It models the lifecycle of liquidity migration requests between:
 *
 *      Solana <-> Pons Gateway <-> Robinhood Chain
 *
 ...
 *
 * Pons Gateway is designed around the same modular philosophy used throughout
 * the Pons V2 architecture:
 *
...
contract PonsGateway {
    ...
    bool public gatewayActive = true;
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "PonsGateway: not owner");
        _;
    }

    modifier onlyRelayer() {
        require(msg.sender == relayer, "PonsGateway: not relayer");
        _;
    }

    modifier onlyGuardian() {
        require(msg.sender == guardian, "PonsGateway: not guardian");
        _;
    }

    modifier gatewayOpen() {
        require(gatewayActive, "PonsGateway: paused");
        _;
    }
     ...
     *
     * The gateway records the Robinhood token and the destination Solana mint.
        ...
        returns (bytes32 migrationId)
    {
        require(
            ...
            "PonsGateway: token unsupported"
        );

        require(
            supportedSolanaMints[solanaMint],
            "PonsGateway: mint unsupported"
        );

        require(amount > 0, "PonsGateway: zero amount");
        ...
        external
    {
        ...
        require(
            ...
            msg.sender == guardian,
            "PonsGateway: not authorized"
        );

        require(
            migration.status == MigrationStatus.REQUESTED ||
            migration.status == MigrationStatus.ATTESTED,
            "PonsGateway: cannot cancel"
        );

        require(
            block.timestamp >=
                migration.createdAt + CANCELLATION_DELAY,
            "PonsGateway: cancellation locked"
        );
